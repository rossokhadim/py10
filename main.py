...
new_list = list(filter(lambda x: float(x['rating'])>8.5, imdb_rank))
new_list = [val['title'] for val in list(sorted(new_list, key = lambda x: x['rating']))]
print(new_list)

def search(name):
  for film in imdb_rank:
    if(name == film['title']):return film['trailer']
...
def search1(name):
  ...
  film_names = []
  for film in imdb_rank:
    if((name in film['director']) or (name in film['writers'])):
      film_num+=1
      rating_list.append(film['rating'])
      film_names.append(film['title'])
  ave_raiting = 0
  for i in rating_list:
    ave_raiting+=float(i)
  ave_rating = 0
  if(len(rating_list)!=0): ave_raiting/len(rating_list)
  actor_name.update({'Name': name})
  actor_name.update({'Number of films': film_num})
  actor_name.update({'Rating': rating_list})
  actor_name.update({'Average rating': ave_rating})
  actor_name.update({'Films titles': film_names})
  return actor_name


Name = input()
Dict_actor = search1(Name)
print(Dict_actor)
